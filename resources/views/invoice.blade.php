...
@include("include/profileSideMenu")
    <div class="col-9">
        @if(count($InvioceAddress) == 0)
            <div class="empty-cart">Még nem adtál meg szállítási címet!</div>
        @else
            @foreach ($InvioceAddress as $object)
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-4">
                                <a href="/invoice/{{ $object->id }}" class="btn-cart">Módosítás</a>
                                <form action="{{ route('shop.deleteInvoice') }}" method="post">
                                    @csrf
    ...
    @else
    <form action="{{ route('shop.modifyInvoices') }}" method="post" class="col-3 sum-cart-div">
        @csrf
            ...
            <div class="">
                ...
                <select class="form-control" name="street_type">
                    <option value="">Kérem válasszon</option>
                        ...
                        <option value="{{ $ps_object->ps_id }}" 
                            @if($ps_object->ps_id == $modify_address->street_type)
                                selected
                            @endif    
